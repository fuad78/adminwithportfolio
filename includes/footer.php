        ...
        <div class="max-w-7xl mx-auto px-4 flex flex-col items-center space-y-4">
            <div class="flex justify-center space-x-6">
                ...
                while ($row = $result->fetch_assoc()) {
                    $icon = $icons[$row['icon']] ?? '';
                    echo '<a href="' . htmlspecialchars($row['url']) . '" target="_blank" rel="noopener noreferrer" class="text-white transition-transform transform hover:scale-110 hover:text-blue-400">' . $icon . '</a>';
                }
                $conn->close();
                ?>
            </div>
            <div class="text-gray-400">
                © <?php echo date('Y'); ?> KM Fuad Hasan. All rights reserved.
            </div>
        </div>
    </footer>
</body>
