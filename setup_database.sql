...
-- Create Database
CREATE DATABASE IF NOT EXISTS fuaditme_portfolio_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- NOTE: Users and grants are handled outside this script when running in Docker.
-- If you run this manually on a standalone MySQL server, create the user with:
--   CREATE USER IF NOT EXISTS 'fuaditme_portfolio_user'@'localhost' IDENTIFIED BY 'Portfolio@2024';
--   GRANT ALL PRIVILEGES ON fuaditme_portfolio_db.* TO 'fuaditme_portfolio_user'@'localhost';
--   FLUSH PRIVILEGES;

-- Use the database
USE fuaditme_portfolio_db;
...
CREATE TABLE IF NOT EXISTS password_reset_tokens (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_password_reset_user FOREIGN KEY (admin_id) REFERENCES admin_users(id) ON DELETE CASCADE
);
